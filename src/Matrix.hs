...
-- The method of gaussian elimination known as "pivot condensation" consists of
-- repeating the following until every row has been processed:
-- * Find the row i with largest absolute value in column 0.
-- * Swap rows i and 0 if they are different. The entry in row 0 column 0 is
--   know as the "pivot".
-- * Scale row 0 by (1/a) where a is the pivot, so the pivot becomes 1.
-- * Linearly combine all rows k ≠ 0 so that  row_k' = row_k - a * row_0 where a
...
-- started with an n x n+1 matrix, this will lead to either infinitely many or no
